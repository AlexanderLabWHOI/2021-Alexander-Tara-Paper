...
\begin{landscape}
    ...
    \end{figure}
\end{landscape}
% Why are all the SI figures below after the references?

%Notes on tree that will eventually be here
%https://itol.embl.de/tree/12812810207359791620499625#
% When we take a look at a subset of TOPAZ MAGs, mix in ok with both MAGs and SAGs from previous \textit{Tara} ocean efforts.
% We cannot necessarily annotate beyond SAR, but with such a large group that likely dominantes most marine microbiomes, we can get much further than previous analyses with respect to what members within SAR exist. 
% they do not seem to be in culture, as they were not annotated beyond SAR, but we can make important ecological considerations about them. 
% Tree to do/chat with HA
%% singletons? random MAGs that are dinophyceae? NA? ciliates? other things that are in the middle of a bunch of diatoms. Remove?
%% Choanoflagelaltes in 2 places.. why would this happen? 
% Upon collapsing... show (1) # of diatoms vs other nad (2) # of Alexander versus Delmont

% TO DO:
% MAST are recovered, but they are not these ones.
% we have choanos ?
